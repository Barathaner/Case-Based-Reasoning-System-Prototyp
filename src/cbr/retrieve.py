    ...
    for key in constraints:
        ...
        if key == "ingredients":
            if len(constraints[key]['include']['name'])>0 or len(constraints[key]['exclude']['name'])>0:
                update_sim2(sim,cumulative_norm_score,constraints,'name','ingr_name_match',recipe_ingredients)
            if len(constraints[key]['include']['basic_taste'])>0 or len(constraints[key]['exclude']['basic_taste'])>0:
                update_sim2(sim,cumulative_norm_score,constraints,'basic_taste','ingr_basic_taste_match',recipe_ingredients)
            if len(constraints[key]['include']['food_category'])>0 or len(constraints[key]['exclude']['food_category'])>0:
                update_sim2(sim,cumulative_norm_score,constraints,'food_category','ingr_food_cat_match',recipe_ingredients)
            else:  
